...
ИСПРАВЛЕНО:
  1. yaw_source='imu' (гироскоп+акселерометр) вместо 'mag' — компас в монтаже
     «залипает» (не отслеживает повороты), гироскоп отслеживает. Компас
     выключен (use_magnetometer=False).
...
def generate_launch_description():
    ...
    # Нода читает I2C (MPU6050) и публикует /odom и /imu/data.
    # ВАЖНО: yaw_source='imu' (гироскоп+акселерометр) — отслеживает повороты.
    # Компас выключен (use_magnetometer=False) — «залипает» в этом монтаже.
    odom_node = Node(
        package='robot_odom',
        ...
        parameters=[{
            ...
            # --- КУРС: гироскоп, компас ВЫКЛ ---
            'yaw_source': 'imu',           # гироскоп+акселерометр (основной)
            'use_magnetometer': False,     # компас выключен (залипает)
