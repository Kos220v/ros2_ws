...
Датчик MPU6050 не отвечает по I2C. Проверьте по порядку:

```bash
sudo i2cdetect -y 1     # должен быть виден адрес 0x68
```

Нет `0x68` — дело в питании или проводах датчика. Есть `0x68` — смотрите
лог запуска: узел `mpu6050_control` сам повторяет попытки подключения
каждые 5 секунд и печатает причину отказа.
