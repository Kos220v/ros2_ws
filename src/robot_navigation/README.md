...
**2. Высокая частота шины.** Чем больше устройств, тем выше ёмкость линий
и тем хуже фронты сигналов. Три устройства (`0x0D`, `0x68`, `0x76`) — уже
заметная нагрузка. Снизьте частоту до 100 кГц:

```bash
sudo nano /boot/firmware/config.txt
```

Найдите строку `dtparam=i2c_arm=on` и приведите её к виду:

```
dtparam=i2c_arm=on,i2c_arm_baudrate=100000
```

Затем `sudo reboot`. Это самая частая аппаратная причина плавающих
отказов I2C на Raspberry Pi.

**3. Провода.** Длинные или неэкранированные SDA/SCL рядом с силовыми
проводами моторов. Помогает укоротить, свить с землёй или увести от
силовой части.
