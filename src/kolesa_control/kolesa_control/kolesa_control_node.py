...
Публикации:
  ...
  /kolesa/yaw         std_msgs/Float64
      Поворот робота вокруг оси Z (рад), нормализован в диапазон [-pi, pi].
      Вычисляется НЕ через геометрическую кинематику (wheel_separation),
      а из total_abs_counts каждого колеса по ЭМПИРИЧЕСКИ измеренной
      зависимости "тики тахометра <-> угол поворота в градусах"
      (см. параметры turn_counts_per_rad_left/right).
"""
...
def velocity_to_duty_cycle(velocity, max_velocity, duty_min=0.03, duty_max=1.0):
    ...
    if abs(velocity) < 0.001:
        return 0.0
    # Работаем с модулем скорости для расчета величины скважности
    normalized = abs(velocity) / max_velocity
    normalized = min(1.0, max(0.0, normalized))
    duty_magnitude = duty_min + normalized * (duty_max - duty_min)
    # Возвращаем скважность с правильным знаком скорости
...
class KolesaControl(Node):
    def __init__(self):
        ...
        # Если при invert_left/invert_right команда поворота (angular.z)
        # даёт поворот в противоположную сторону (или вообще не поворачивает,
        # потому что оба борта уходят в одну сторону) — поставьте True.
        # Инвертирует знак угловой скорости ДО разложения на борта,
        # не трогая линейное движение.
        ...
        # Эффективный радиус колеса/звездочки, м
        ...
        # Производные коэффициенты пересчета тиков
        ...
        self.get_logger().info("КОНФИГУРАЦИЯ ПРИВОДА (ИСПРАВЛЕННАЯ, DUTY CYCLE)")
        ...
        self.get_logger().info(
            ...
            f"публикация в /kolesa/yaw: {self.pub_yaw}"
        )
        ...
        if self.pub_yaw:
            self.yaw_pub = self.create_publisher(Float64, "kolesa/yaw", 10)
    ...
    def _control_tick(self):
        ...
        # ИСПРАВЛЕНИЕ ПОВОРОТА:
        # Если invert_left/invert_right искажают знак поворота (оба борта
        # уходят в одну сторону при w!=0), инвертируем w ДО разложения.
        if self.invert_angular:
            w = -w

        half_sep = self.separation / 2.0

        # Разделение линейной и угловой скоростей по бортам
        ...
        # Преобразуем скорости в скважность (duty cycle)
        ...
        # Отправляем команды драйверам VESC
    ...
    def _update_robot_pose(self):
        ...
        # Поступательное перемещение центра робота
        distance_left = (
        ...
        # --- ИСПРАВЛЕНИЕ: инвертирован знак d_theta ---
        # Вычисление приращения угла yaw (рад) по эмпирической зависимости тиков.
        # Движение правого борта вперед уходит в +, левого — в -.
        # ИСПРАВЛЕНО: знак инвертирован для ROS-совместимости
        d_theta = 0.5 * (
            (delta_left / self.turn_counts_per_rad_left)
            - (delta_right / self.turn_counts_per_rad_right)
        )

        # Интегрирование позы 2D методом средней точки (Midpoint integration)
    ...
    def _publish_yaw(self):
        """Публикует текущий поворот робота вокруг оси Z (рад, -pi..pi)."""
        if self.theta_z is None:
            return
        msg = Float64()
        msg.data = normalize_angle(self.theta_z)
